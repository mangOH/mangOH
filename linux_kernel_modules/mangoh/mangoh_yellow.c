...
static int mangoh_yellow_probe(struct platform_device* pdev)
{
	...
	*/
	...
	mangoh_yellow_driver_data.eeprom =
		i2c_new_device(main_adapter, &mangoh_yellow_eeprom_info);
	...
	mangoh_yellow_driver_data.i2c_switch = 
		i2c_new_device(main_adapter, &mangoh_yellow_pca954x_device_info);
	...
	other_adapter = i2c_get_adapter(MANGOH_YELLOW_I2C_BUS_PORT3);
	if (!other_adapter) {
		dev_err(&pdev->dev,
			"Couldnt get the I2C bus %d to add the GPIO expander.\n",
			MANGOH_YELLOW_I2C_BUS_PORT3);
		ret = -ENODEV;
		goto cleanup;
	}
	mangoh_yellow_driver_data.gpio_expander = 
		i2c_new_device(other_adapter, &mangoh_yellow_gpio_expander_devinfo);
	...
	dev_dbg(&pdev->dev, "mapping bme680 gas/temperature/pressure sensor\n");    
        other_adapter = i2c_get_adapter(MANGOH_YELLOW_I2C_BUS_PORT1);
	if (!other_adapter) {
		dev_err(&pdev->dev,
			"Couldn't get the I2C bus %d to add the Environmental sensor \n",
			MANGOH_YELLOW_I2C_BUS_PORT1);
		ret = -ENODEV;
		goto cleanup;
	}
	mangoh_yellow_driver_data.environmental =
		i2c_new_device(other_adapter, &mangoh_yellow_environmental_devinfo);
	...
	if(!other_adapter) {
		dev_err(&pdev->dev,
			"Couldn't get the I2C bus %d to add the IMU sensor \n",
			MANGOH_YELLOW_I2C_BUS_PORT1);
			ret = -ENODEV;
			goto cleanup;
	}
        mangoh_yellow_driver_data.imu =
		i2c_new_device(other_adapter, &mangoh_yellow_imu_devinfo);
        i2c_put_adapter(other_adapter);
	...
	if(!other_adapter) {
		dev_err(&pdev->dev,
			"Couldn't get the I2C bus %d to add the magentometer sensor \n",
			MANGOH_YELLOW_I2C_BUS_PORT1);
			ret = -ENODEV;
			goto cleanup;
	}
        mangoh_yellow_driver_data.magnetometer =
		i2c_new_device(other_adapter, &mangoh_yellow_magnetometer_devinfo);
        i2c_put_adapter(other_adapter);
	if (!mangoh_yellow_driver_data.imu) {
	...
	if(!other_adapter) {
		dev_err(&pdev->dev,
			"Couldn't get the I2C bus %d to add the RTC \n",
			MANGOH_YELLOW_I2C_BUS_PORT3);
			ret = -ENODEV;
			goto cleanup;
	}
        mangoh_yellow_driver_data.rtc =
		i2c_new_device(other_adapter, &mangoh_yellow_rtc_devinfo);
        i2c_put_adapter(other_adapter);
	...
	other_adapter = i2c_get_adapter(MANGOH_YELLOW_I2C_BUS_PORT2);
	if(!other_adapter) {
		dev_err(&pdev->dev,
			"Couldn't get the I2C bus %d to add the Battery Charger \n",
			MANGOH_YELLOW_I2C_BUS_PORT2);
			ret = -ENODEV;
			goto cleanup;
	}
        mangoh_yellow_driver_data.battery_charger =
		i2c_new_device(other_adapter, &mangoh_yellow_battery_charger_devinfo);
        i2c_put_adapter(other_adapter);
	...
	if(!other_adapter) {
		dev_err(&pdev->dev,
			"Couldn't get the I2C bus %d to add the Battery Gauge\n",
			MANGOH_YELLOW_I2C_BUS_PORT2);
			ret = -ENODEV;
			goto cleanup;
	}
        mangoh_yellow_driver_data.battery_charger =
		i2c_new_device(other_adapter, &mangoh_yellow_battery_gauge_devinfo);
        i2c_put_adapter(other_adapter);
	...
	if(!other_adapter) {
		dev_err(&pdev->dev,
			"Couldn't get the I2C bus %d to add the light sensor \n",
			MANGOH_YELLOW_I2C_BUS_PORT2);
			ret = -ENODEV;
			goto cleanup;
	}
	...
	mangoh_yellow_driver_data.light =
		i2c_new_device(other_adapter, &mangoh_yellow_light_devinfo);
        i2c_put_adapter(other_adapter);
...
static int mangoh_yellow_remove(struct platform_device* pdev)
{
	...
	dev_info(&pdev->dev, "Removing mangoh yellow platform device\n");
        ...
        try_unregister_i2c_device(dd->environmental);
        try_unregister_i2c_device(dd->imu);
	try_unregister_i2c_device(dd->magnetometer);
        try_unregister_i2c_device(dd->light);
	try_unregister_i2c_device(dd->battery_charger);
	try_unregister_i2c_device(dd->battery_gauge);
	try_unregister_i2c_device(dd->rtc);
