...
static int mangoh_red_probe(struct platform_device* pdev)
{
	...
	mangoh_red_driver_data.battery_gauge = i2c_new_device(adapter, &mangoh_red_battery_gauge_devinfo);
	...
	if(!adapter) {
        ...
        mangoh_red_driver_data.battery_charger = i2c_new_device(adapter, &mangoh_red_battery_charger_devinfo);
        if (!mangoh_red_driver_data.battery_gauge) {
...
static int mangoh_red_remove(struct platform_device* pdev)
{
	...
	i2c_unregister_device(mangoh_red_driver_data.battery_charger);
	i2c_put_adapter(mangoh_red_driver_data.battery_charger->adapter);

	i2c_unregister_device(mangoh_red_driver_data.battery_gauge);
	i2c_put_adapter(mangoh_red_driver_data.battery_gauge->adapter);
