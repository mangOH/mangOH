...
NfcTag MifareClassic::read(byte *uid, unsigned int uidLength)
{
    ...
    if (success)
    {
        ...
        {
            LE_INFO("Error. Failed read block ");LE_INFO(currentBlock);
            return NfcTag(uid, uidLength, MIFARE_CLASSIC);
        }
    }
    else
    {
        LE_INFO("Tag is not NDEF formatted.");
    ...
    LE_INFO("Message Length ");LE_INFO(messageLength);
    LE_INFO("Buffer Size ");LE_INFO(bufferSize);
    #endif
    ...
    while (index < bufferSize)
    {
        ...
        if (_nfcShield->mifareclassic_IsFirstBlock(currentBlock))
        {
            ...
            if (!success)
            {
                LE_INFO("Error. Block Authentication failed for ");LE_INFO(currentBlock);
                // TODO error handling
            }
        }
        ...
        {
            #ifdef MIFARE_CLASSIC_DEBUG
            LE_INFO("Block ");LE_INFO(currentBlock);LE_INFO(" ");
            _nfcShield->PrintHexChar(&buffer[index], BLOCK_SIZE);
            #endif
        }
        else
        {
            LE_INFO("Read failed ");LE_INFO(currentBlock);
        ...
        {
            #ifdef MIFARE_CLASSIC_DEBUG
            LE_INFO("Skipping block ");LE_INFO(currentBlock);
            #endif
            currentBlock++;
        }
    }
...
int MifareClassic::getNdefStartIndex(byte *data)
{
    ...
    for (int i = 0; i < BLOCK_SIZE; i++)
    {
        ...
        {
            LE_INFO("Unknown TLV ");LE_INFO(data[i], HEX);
            return -2;
        }
    }
...
bool MifareClassic::formatNDEF(byte * uid, unsigned int uidLength)
{
    ...
    if (!success)
    {
        LE_INFO("Unable to authenticate block 0 to enable card formatting!");
        return false;
    }
    success = _nfcShield->mifareclassic_FormatNDEF();
    if (!success)
    {
        LE_INFO("Unable to format the card for NDEF");
    }
    else
    {
        for (int i=4; i<64; i+=4) {
            ...
            if (success) {
                if (i == 4)  // special handling for block 4
                {
                    ...
                    {
                        LE_INFO("Unable to write block ");LE_INFO(i);
                    }
                }
                else
                {
                ...
                {
                    LE_INFO("Unable to write block ");LE_INFO(i+1);
                }
                if (!(_nfcShield->mifareclassic_WriteDataBlock (i+2, sectorbuffer0)))
                {
                    LE_INFO("Unable to write block ");LE_INFO(i+2);
                }
                if (!(_nfcShield->mifareclassic_WriteDataBlock (i+3, sectorbuffer4)))
                {
                    LE_INFO("Unable to write block ");LE_INFO(i+3);
                }
            } else {
                unsigned int iii=uidLength;
                LE_INFO("Unable to authenticate block ");LE_INFO(i);
...
bool MifareClassic::formatMifare(byte * uid, unsigned int uidLength)
{
    ...
    for (idx = 0; idx < numOfSector; idx++)
    {
        ...
        if (!success)
        {
            LE_INFO("Authentication failed for sector "); LE_INFO(idx);
            return false;
        }
        ...
        if (idx == 0)
        {
            ...
            {
                LE_INFO("Unable to write to sector "); LE_INFO(idx);
            }
        }
        else
        {
        ...
        {
            LE_INFO("Unable to write trailer block of sector "); LE_INFO(idx);
        }
    }
    return true;
}
...
bool MifareClassic::write(NdefMessage& m, byte * uid, unsigned int uidLength)
{
    ...
    #ifdef MIFARE_CLASSIC_DEBUG
    LE_INFO("sizeof(encoded) "));LE_INFO(sizeof(encoded);
    LE_INFO("sizeof(buffer) "));LE_INFO(sizeof(buffer);
    #endif
    ...
    while (index < sizeof(buffer))
    {
        ...
        {
            #ifdef MIFARE_CLASSIC_DEBUG
            LE_INFO("Wrote block ");LE_INFO(currentBlock);LE_INFO(" - ");
            _nfcShield->PrintHexChar(&buffer[index], BLOCK_SIZE);
            #endif
        }
        else
        {
            LE_INFO("Write failed ");LE_INFO(currentBlock);
            return false;
        }
        index += BLOCK_SIZE;
        currentBlock++;
