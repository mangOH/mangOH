...
{
    //LE_INFO("NdefRecord Constructor 1");
    _tnf = 0;
    _typeLength = 0;
    _payloadLength = 0;
    _idLength = 0;
...
{
    //LE_INFO("NdefRecord Constructor 2 (copy)");
...
{
    LE_INFO("NDEF Record     TNF: %0x ", _tnf);
    switch (_tnf) {
    case TNF_EMPTY:
        LE_INFO("Empty");
        break;
    ...
    LE_INFO("Type Length %0x", _typeLength);
    LE_INFO("Payload Length %0x", _payloadLength);
    if (_idLength)
    {
        LE_INFO("Id Length %0x", _idLength);
    }
    LE_INFO("Type:");PrintHexChar(_type, _typeLength);
    // TODO chunk large payloads so this is readable
    LE_INFO("Payload:");PrintHexChar(_payload, _payloadLength);
    if (_idLength)
    {
        LE_INFO("Id:");PrintHexChar(_id, _idLength);
    }
    LE_INFO("Record is %d bytes", getEncodedSize());
